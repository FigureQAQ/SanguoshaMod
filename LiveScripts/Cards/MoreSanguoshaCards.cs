...
internal static class SanguoshaCardFx
{
    ...
    public static Task Damage(PlayerChoiceContext context, CardPlay play, Creature target, decimal amount)
    {
        return amount <= 0 || !target.IsAlive
            ? Task.CompletedTask
            : CreatureCmd.Damage(context, target, amount, ValueProp.Move, play.Card.Owner.Creature, play.Card, play);
    }
        ...
        int minCards = 0)
    {
        ...
        foreach (var card in selected)
        {
            ...
            await SanguoshaCharacterSkills.OnCardExhaustedFromHand(player, card, play.Card);
        }
    ...
    public static int ReduceHandCardCosts(CardPlay play, int count, int amount, Func<CardModel, bool>? filter = null)
    {
        var changed = 0;
        foreach (var card in play.Card.Owner.PlayerCombatState!.Hand.Cards
                     .Where(card => card != play.Card
                         && card.EnergyCost.GetResolved() > 0
                         && (filter?.Invoke(card) ?? true))
                     .OrderByDescending(card => card.EnergyCost.GetResolved()))
        {
    ...
    public static async Task<int> ClearBuffs(Creature creature)
    {
        var buffs = creature.Powers.Where(power => power.Type == PowerType.Buff).ToList();
    ...
    public static async Task<bool> ClearOneBuff(Creature creature)
    {
        var buff = creature.Powers.FirstOrDefault(power => power.Type == PowerType.Buff);
        if (buff is null)
        {
            return false;
        }
    ...
    public static async Task<int> StealBuffsThisTurn(PlayerChoiceContext context, CardPlay play, Creature target)
    {
        var player = play.Card.Owner;
        var buffs = target.Powers
            .Where(power => power.Type == PowerType.Buff && power.Amount > 0)
            .ToList();
        foreach (var buff in buffs)
        {
            var amount = buff.Amount;
            var copiedPower = (PowerModel)buff.ClonePreservingMutability();
            await PowerCmd.Remove(buff);
            await PowerCmd.Apply(context, copiedPower, player.Creature, amount, player.Creature, play.Card, false);
            SanguoshaCharacterSkills.RegisterTemporaryStolenBuff(player, copiedPower.Id, amount);
        }
    ...
    public static async Task Heal(Player player, decimal amount)
    {
        ...
        if (amount > 0 && missing > 0)
        {
            ...
            SanguoshaCharacterSkills.MarkHealed(player);
        }
    }
    ...
    public static Task Vulnerable(PlayerChoiceContext context, CardPlay play, Creature target, decimal amount)
    {
        ...
        return ApplyVulnerable(context, play, target, amount);
    }

    private static async Task ApplyVulnerable(PlayerChoiceContext context, CardPlay play, Creature target, decimal amount)
    {
        await PowerCmd.Apply<VulnerablePower>(context, target, amount, play.Card.Owner.Creature, play.Card, false);
        await SanguoshaCharacterSkills.OnVulnerableApplied(play.Card.Owner, target, amount, play.Card);
    }
    ...
    public static Task Thunder(PlayerChoiceContext context, CardPlay play, decimal amount)
    {
        return amount <= 0
            ? Task.CompletedTask
            : PowerCmd.Apply<ThunderPower>(
                context,
                play.Card.Owner.Creature,
                amount,
                play.Card.Owner.Creature,
                play.Card,
                false);
...
public sealed class BaiYinCard : SanguoshaCard
{
    protected override IEnumerable<DynamicVar> CanonicalVars => [
        new DynamicVar("DamageCap", 20m),
        new DynamicVar("LeaveHeal", 10m)
    ];

    public BaiYinCard() : base(2, CardType.Power, CardRarity.Rare, TargetType.Self)
    {
    }
    ...
    protected override void OnUpgrade()
    {
        ...
        DynamicVars["DamageCap"].UpgradeValueBy(-5);
    }
}
...
public sealed class BingLiangCard : SanguoshaCard
{
    ...
    protected override Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        SanguoshaCharacterSkills.AddDelayedJudgment(
            cardPlay.Card.Owner,
            cardPlay.Target!,
            DelayedJudgmentKind.BingLiang,
            0,
            cardPlay.Card);
        return Task.CompletedTask;
    }
...
public sealed class ChiTuCard : SanguoshaCard
{
    ...
    public ChiTuCard() : base(1, CardType.Power, CardRarity.Uncommon, TargetType.Self)
    {
    }
...
public sealed class GuDingCard : SanguoshaCard
{
    ...
    protected override void OnUpgrade()
    {
        DynamicVars["AttackBonus"].UpgradeValueBy(1);
    }
}
...
{
    protected override IEnumerable<DynamicVar> CanonicalVars => [
        new DynamicVar("SlowCap", 50m),
        new BlockVar(10, ValueProp.Move)
    ];

    public TengJiaCard() : base(2, CardType.Power, CardRarity.Uncommon, TargetType.Self)
    {
    }
    ...
    {
        EnergyCost.SetCustomBaseCost(1);
        DynamicVars["SlowCap"].UpgradeValueBy(-20);
    }
}
...
public sealed class MengDeXinShuCard : SanguoshaCard
{
    ...
    public MengDeXinShuCard() : base(2, CardType.Power, CardRarity.Uncommon, TargetType.Self)
    {
    }
...
public sealed class GuoHeCard : SanguoshaCard
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        var removed = await SanguoshaCardFx.ClearOneBuff(cardPlay.Target!);
        if (removed && IsUpgraded)
        {
            await SanguoshaCardFx.Draw(choiceContext, cardPlay, DynamicVars["Draw"].IntValue);
        }
    }
...
public sealed class HuoGongCard : SanguoshaCard
{
    ...
    public HuoGongCard() : base(1, CardType.Skill, CardRarity.Common, TargetType.AnyEnemy)
    {
    }
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        var damage = DynamicVars.Damage.BaseValue
            + (discarded is not null ? DynamicVars["BonusDamage"].BaseValue : 0m);
...
public sealed class NanManCard : SanguoshaCard
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        foreach (var target in SanguoshaCardFx.AliveEnemies(cardPlay))
        {
...
public sealed class QiLinCard : SanguoshaCard
{
    ...
    public QiLinCard() : base(2, CardType.Power, CardRarity.Uncommon, TargetType.Self)
    {
    }
...
public sealed class QingGangCard : SanguoshaCard
{
    ...
    public QingGangCard() : base(2, CardType.Power, CardRarity.Uncommon, TargetType.Self)
    {
    }
...
{
    protected override IEnumerable<DynamicVar> CanonicalVars => [
        new DynamicVar("DamagePercent", 8m)
    ];

    protected override bool HasEnergyCostX => true;

    public ShanDianCard() : base(-1, CardType.Skill, CardRarity.Rare, TargetType.AnyEnemy)
    {
    }
...
public sealed class ShunShouCard : SanguoshaCard
{
    ...
    {
        var target = cardPlay.Target!;
        var stolen = await SanguoshaCardFx.StealBuffsThisTurn(choiceContext, cardPlay, target);
        if (stolen > 0)
        {
            SanguoshaCardFx.GainEnergy(cardPlay, DynamicVars.Energy.IntValue);
        }
...
public sealed class TaoYuanCard : SanguoshaCard
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        foreach (var player in SanguoshaCardFx.AliveCombatPlayers(cardPlay))
        {
            ...
            await CreatureCmd.GainBlock(player.Creature, DynamicVars.Block.BaseValue, ValueProp.Move, cardPlay, false);
