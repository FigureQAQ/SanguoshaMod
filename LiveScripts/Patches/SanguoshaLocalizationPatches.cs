...
internal static class SanguoshaLocalization
{
    ...
    private static readonly IReadOnlyDictionary<string, Entry> Text = new Dictionary<string, Entry>
    {
        ...
        ["SANGUOSHA_CARD_WANG_JIAN_SHA_CARD.description"] = new("cards", "由铸剑生成。视为杀造成 {Damage:diff()} 点伤害，并额外造成等同于号令的伤害；不会触发储能杀的铸剑效果。"),
        ...
        ["SANGUOSHA_CARD_TENG_JIA_CARD.description"] = new("cards", "装备：防具。下回合开始，每回合获得 {Block:diff()} 点格挡并获得 {Slow:diff()} 层减速。失去时获得格挡。"),
        ...
        ["SANGUOSHA_CARD_YI_JI_CARD.description"] = new("cards", "能力牌。抽 1 张牌。每回合首次敌人即将对你造成可被格挡的伤害时，抽 {Draw:diff()} 张牌；若触发前手牌不超过 3 张，额外抽 {LowHandDraw:diff()} 张牌。然后使手牌中至多 {FreeCards:diff()} 张杀或技能牌本回合变为 0 费。"),
        ...
        ["SANGUOSHA_POWER_TENG_JIA_DISPLAY_POWER.description"] = new("powers", "装备：防具。回合开始时获得 10 点格挡并获得 1 层减速。失去时获得格挡。"),
        ...
        ["SANGUOSHA_POWER_YI_JI_DISPLAY_POWER.description"] = new("powers", "能力牌。每回合首次敌人即将对你造成可被格挡的伤害时抽牌；若触发前手牌不超过 3 张，额外抽牌并临时减费。"),
        ...
        ["SANGUOSHA_POWER_IRONCLAD_SKILL_DISPLAY_POWER.title"] = new("powers", "武魂：火杀"),
        ["SANGUOSHA_POWER_IRONCLAD_SKILL_DISPLAY_POWER.description"] = new("powers", "杀命中后追加火焰伤害；生命不高于一半时，每回合首次火杀会更强并施加易伤。"),
        ["SANGUOSHA_POWER_SILENT_SKILL_DISPLAY_POWER.title"] = new("powers", "武魂：毒杀"),
        ["SANGUOSHA_POWER_SILENT_SKILL_DISPLAY_POWER.description"] = new("powers", "杀命中后施加中毒。毒杀命中已有中毒的敌人时，每回合首次获得能量。"),
        ["SANGUOSHA_POWER_DEFECT_SKILL_DISPLAY_POWER.title"] = new("powers", "武魂：雷杀"),
        ["SANGUOSHA_POWER_DEFECT_SKILL_DISPLAY_POWER.description"] = new("powers", "杀命中后积累雷势并造成连锁伤害；雷势达到阈值时释放群体伤害。"),
        ["SANGUOSHA_POWER_NECROBINDER_SKILL_DISPLAY_POWER.title"] = new("powers", "武魂：灾厄杀"),
        ["SANGUOSHA_POWER_NECROBINDER_SKILL_DISPLAY_POWER.description"] = new("powers", "杀命中后施加杀数值一半的灾厄并获得魂值。打出闪时所有敌人失去 3 点生命；魂值满 4 点时消耗魂值触发治疗和补牌。"),
        ["SANGUOSHA_POWER_REGENT_SKILL_DISPLAY_POWER.title"] = new("powers", "武魂：储能杀"),
        ["SANGUOSHA_POWER_REGENT_SKILL_DISPLAY_POWER.description"] = new("powers", "杀命中后积累铸剑；铸剑达到阈值时生成 0 费王剑杀。星辉达到阈值时发动诏令。"),

        ["IRONCLAD.title"] = new("characters", "赤壁猛将"),
        ["IRONCLAD.titleObject"] = new("characters", "赤壁猛将"),
        ["IRONCLAD.description"] = new("characters", "武魂：火杀。定位：攻击强化与低生命爆发。"),
        ["SILENT.title"] = new("characters", "锦囊夜行"),
        ["SILENT.titleObject"] = new("characters", "锦囊夜行"),
        ["SILENT.description"] = new("characters", "武魂：毒杀。定位：中毒叠加、技能牌联动与回能。"),
        ["DEFECT.title"] = new("characters", "机关卧龙"),
        ["DEFECT.titleObject"] = new("characters", "机关卧龙"),
        ["DEFECT.description"] = new("characters", "武魂：雷杀。定位：雷势积累、连锁伤害与群体爆发。"),
        ["NECROBINDER.title"] = new("characters", "青囊魂医"),
        ["NECROBINDER.titleObject"] = new("characters", "青囊魂医"),
        ["NECROBINDER.description"] = new("characters", "武魂：灾厄杀。定位：治疗续航、魂值循环与闪避反击。"),
        ["REGENT.title"] = new("characters", "汉室仁主"),
        ["REGENT.titleObject"] = new("characters", "汉室仁主"),
        ["REGENT.description"] = new("characters", "武魂：储能杀。定位：号令成长、星辉诏令与团队节奏。"),
        ...
        ["SANGUOSHA_RELIC_SILENT_SKILL_RELIC.description"] = new("relics", "战斗开始：抽 1 张牌并获得 2 点机巧。杀附魔：毒杀，命中后施加 2 层中毒；若目标原本没有中毒，额外施加 1 层。每回合首次打出技能牌强化下一张毒杀；每回合首次命中已中毒目标时获得 1 点能量。"),
        ...
        ["SANGUOSHA_RELIC_DEFECT_SKILL_RELIC.description"] = new("relics", "战斗开始：获得 1 点雷势。杀附魔：雷杀，命中后获得 1 点雷势，并对目标追加 1 点雷击、对其他敌人追加 2 点雷击。雷势达到 4 时消耗 4 点，对所有敌人造成 8 点伤害；每回合最多触发 1 次。"),
        ...
        ["SANGUOSHA_RELIC_REGENT_SKILL_RELIC.description"] = new("relics", "战斗开始：获得 1 点号令和 1 点星辉。杀附魔：储能杀，命中后获得铸剑；铸剑达到 3 时生成 1 张 0 费王剑杀。星辉达到 3 时每回合最多发动 1 次诏令：获得能量、抽牌、提升号令、推进铸剑，并使 1 张杀或技能牌本回合免费。"),
